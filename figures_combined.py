...
def plot_robustness_results(raw_df, output_filename, title, xlabel, model_types, invert_xaxis=False, xticklabels=None):
    ...
    raw_df_copy.rename(columns={'Label': 'Combined Error Level'}, inplace=True)
    
    grouped_main = raw_df_copy.groupby(['Combined Error Level', 'Algorithm'])['Mean Sum-Rate'].agg(
        mean='mean', 
        std='std', 
        count='count'
    ).reset_index()
    ...
    grouped_main['ci_95'] = grouped_main.apply(
        lambda row: calculate_ci_95(raw_df_copy[
            (raw_df_copy['Combined Error Level'] == row['Combined Error Level']) & 
            (raw_df_copy['Algorithm'] == row['Algorithm'])
        ]['Mean Sum-Rate']), 
        axis=1
    )
    
    grouped_bf_only = raw_df_copy[raw_df_copy['Mean Sum-Rate (BF-Only)'] > 0].groupby(['Combined Error Level', 'Algorithm'])['Mean Sum-Rate (BF-Only)'].agg(
        mean='mean', 
        std='std', 
        count='count'
    ).reset_index()
    ...
    try:
        if xticklabels:
             results_df['Combined Error Level'] = pd.Categorical(results_df['Combined Error Level'], categories=xticklabels, ordered=True)
             results_df = results_df.sort_values(by='Combined Error Level', ascending=not invert_xaxis)
        else:
             results_df['Combined Error Level'] = pd.to_numeric(results_df['Combined Error Level'])
             results_df = results_df.sort_values(by='Combined Error Level', ascending=not invert_xaxis)
    except Exception:
        pass
    ...
    plot_x_values = results_df['Combined Error Level'].unique()
    ...
    for mt in model_types:
        ...
        if not model_data.empty:
            ...
            ax.errorbar(
                x=model_data['Combined Error Level'],
                y=model_data['mean'], 
                yerr=y_error,
                label=mt, 
                color=color,
...
COMBINED_LEVELS = {
    0: {'rho': 1.0, 'jitter': 0.0, 'label': 'L0'},
    1: {'rho': 0.9, 'jitter': 3.0, 'label': 'L1'},
    2: {'rho': 0.8, 'jitter': 5.0, 'label': 'L2'},
    3: {'rho': 0.6, 'jitter': 7.0, 'label': 'L3'},
    4: {'rho': 0.5, 'jitter': 10.0, 'label': 'L4'}
}
...
def main():
    ...
    RESULTS_DIR = "./paper_figures_combined"
    os.makedirs(RESULTS_DIR, exist_ok=True)
    os.makedirs("./runs_figures/logs", exist_ok=True)
    
    raw_results_csv_path = os.path.join(RESULTS_DIR, "combined_robustness_raw.csv")
    ...
    if os.path.exists(raw_results_csv_path):
        try:
            ...
            for _, row in completed_df.iterrows():
                completed_jobs.add((row['Algorithm'], row['Label'], int(row['Seed'])))
    ...
    for level_config in COMBINED_LEVELS.values():
        label = level_config['label']
        rho = level_config['rho']
        jitter = level_config['jitter']
        
        for model in model_types_to_run:
            for seed in SEEDS_TO_RUN:
                job_tuple = (model, label, seed)
                
                if job_tuple in completed_jobs:
                    print(f"Skipping (cached): {model} @ {label} (Seed {seed})")
                else:
                    print(f"Adding to queue:   {model} @ {label} (Seed {seed})")
                    job_info = (model, label, float(rho), float(jitter), int(seed)) 
    ...
    if not total_jobs:
        ...
        while drl_jobs or cpu_jobs or drl_processes or cpu_processes:
            ...
            while len(drl_processes) < MAX_DRL_RUNS and drl_jobs:
                model, label, rho, jitter, seed = drl_jobs.pop(0)
                cmd = [sys.executable, __file__, "--model", model, "--label", label, "--rho", str(rho), "--jitter", str(jitter), "--seed", str(seed)]
                p = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                drl_processes.append(p)

            while len(cpu_processes) < MAX_CPU_RUNS and cpu_jobs:
                model, label, rho, jitter, seed = cpu_jobs.pop(0)
                cmd = [sys.executable, __file__, "--model", model, "--label", label, "--rho", str(rho), "--jitter", str(jitter), "--seed", str(seed)]
                p = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    plot_robustness_results(
        raw_df_final,
        os.path.join(RESULTS_DIR, "paper_figure_combined_robustness.png"),
        title="Robustness to Combined Error",
        xlabel="Combined Error Level",
        model_types=model_types_to_plot,
        invert_xaxis=False,
        xticklabels=[config['label'] for config in COMBINED_LEVELS.values()]
    )

    print("\n--- Generating All Convergence Plots ---")
    for level_config in COMBINED_LEVELS.values():
        plot_convergence_for_level(
            rho=float(level_config['rho']), 
            jitter=float(level_config['jitter']), 
            base_config=master_config, 
            plot_name_suffix=level_config['label'],
...
if __name__ == '__main__':
    ...
    parser.add_argument("--label", type=str, help="Error level label (e.g., L0, L1)")
    ...
    if args.model:
        if args.label is None or args.rho is None or args.jitter is None or args.seed is None:
        ...
        if float(rho) == 1.0 and float(jitter) == 0.0:
        ...
        try:
            ...
            with FileLock(lock_path):
                ...
                with open(raw_results_csv_path, 'a', newline='') as f:
                    writer = csv.writer(f)
                    if not file_exists:
                        writer.writerow(['Algorithm', 'Label', 'Seed', 'Mean Sum-Rate', 'Mean Sum-Rate (BF-Only)'])
                    ...
                    writer.writerow([model, label, seed, mean_sumrate, mean_sumrate_bf_only])
            
        except Exception as e:
            with open("figure_combined_worker_error.log", "a") as f:
                f.write(f"!!!!!!!! FAILED: {model} @ {label} (Seed {seed}). Error: {e} !!!!!!!!\n")
                import traceback
